"use client";

import Link from "next/link";
import { useT } from "@/lib/i18n";

interface HeaderProps {
  settingsHref?: string;
}

const gearIcon = (
  <svg
    width="20"
    height="20"
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    strokeWidth="2"
    strokeLinecap="round"
    strokeLinejoin="round"
  >
    <circle cx="12" cy="12" r="3" />
    <path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z" />
  </svg>
);

const btnClass =
  "glass flex h-10 w-10 items-center justify-center rounded-full text-white/30 transition hover:text-white/60";

export default function Header({ settingsHref }: HeaderProps) {
  const { t } = useT();

  return (
    <header className="flex items-center justify-between px-5 pb-2" style={{ paddingTop: "max(1rem, env(safe-area-inset-top))" }}>
      <h1 className="font-heading text-2xl font-bold text-glow">Divina</h1>
      {settingsHref ? (
        <Link href={settingsHref} className={btnClass} aria-label={t("header.settings")}>
          {gearIcon}
        </Link>
      ) : (
        <div className="w-10" />
      )}
    </header>
  );
}
