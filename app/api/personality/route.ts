...
import {
  initEphemeris, julianDay, calcAllPlanets, calcHouseCusps,
  getSignIndex, getDegreeInSign, getHouseForPlanet,
  PLANET_NAMES_RU, SIGN_NAMES_RU, SIGN_SYMBOLS, PLANET_SYMBOLS,
} from "@/lib/ephemeris";
import { ASPECT_NAMES } from "@/lib/interpretations";
import { interpretPersonality } from "@/lib/ai-interpret";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sunSign = SIGN_NAMES_RU[getSignIndex(positions[0].longitude)];
    const moonSign = SIGN_NAMES_RU[getSignIndex(positions[1].longitude)];
    const ascendant = SIGN_NAMES_RU[getSignIndex(houses.ascendant)];

    const result = await interpretPersonality({
      birthDate,
      sunSign,
      moonSign,
      ascendant,
